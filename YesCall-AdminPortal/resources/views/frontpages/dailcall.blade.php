...
<div class="calling_body">
    <div class="calling_container">
        ...
        <input type="hidden" value="{{ $number }}" id="phone">
        {{--<p class="calling_page_p">{{ $data->name }}</p>--}}
        <p class="calling_page_p connection_status" id="log">Connecting...</p>
        <p class="calling_page_p connection_timer" id="timer">00:00:00</p>
        <select class="calling_page_p connection_timer" id="package" style="margin-left: 25%;color:black !important">
        ...
        <div class="calling_page_options">
            <button  class="calling_page_buttons" onclick="call()" id="btn-1"><span style="color:white;" class="fa fa-phone fa-2x"></span></button>
            <button  class="calling_page_buttons" id="mute"><span  style="color:white"  class="fa fa-microphone fa-2x" id="mic"></span></button>
            <button  class="calling_page_buttons" onclick="hangup()" id="btn-3"><span style="color:white" class="fa fa-phone fa-2x"></span></button>
        </div>
    </div>
</div>
        ...
        if ($('#package').val() == 'credit') {
            if (credit <= 0) {
                ...
                $.ajax({
                    ...
                    data: {'id': userID, 'country_cost': countryCost},
                ...
                timer.addEventListener('secondsUpdated', function (e) {
                    ...
                    $.ajax({
                        ...
                        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                        data: {id: packageid1, 'time': timer.getTimeValues().toString(), 'country_cost': countryCost},
                        success: function (data) {
                            console.log(data);
                        }
                    ...
                    alert('Sorry! Country is Not Available now.');
                }
            ...
            if(calling_sec<=0){
                ...
                timer.addEventListener('secondsUpdated', function (e) {
                    ...
                    $.ajax({
                        ...
                        headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
                        data:{id:packageid,'time':timer.getTimeValues().toString()},
                        success:function(data){
                            console.log(data);
                        }
